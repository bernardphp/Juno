...
["$scope","Queue",function(a,b){a.queues=b.query()}]);Juno.controller("QueueController",["$scope","$routeParams","Queue",function(a,b,c){a.page=parseInt(b.page)||1;a.pages=1;a.queue={name:b.queue,messages:[]};c.get({queue:b.queue,offset:10*(a.page-1)},function(d){a.pages=Math.ceil(data.count/10);a.queue=d});a["delete"]=function(){c["delete"]({queue:b.queue})}}]);Juno.controller("FailedController",["$controller","$scope","$routeParams","Queue",function(a,b,c,d){c.queue="failed";a("QueueController",
{$scope:b,$routeParams:c,Queue:d})}]);Juno.controller("ConsumersController",["$scope","Consumer",function(a,b){a.consumers=b.query()}]);Juno.config(["$routeProvider","$locationProvider",function(a,b){b.html5Mode(!0);a.when("/",{templateUrl:"template/overview",name:"overview"});a.when("/info",{templateUrl:"template/info",controller:"InfoController",name:"info"});a.when("/queue",{templateUrl:"template/queues",controller:"QueuesController",name:"queue"});a.when("/queue/failed",{templateUrl:"template/queue",
controller:"FailedController",name:"failed"});a.when("/queue/:queue",{templateUrl:"template/queue",controller:"QueueController",name:"queues"});a.when("/consumer",{templateUrl:"template/consumers",controller:"ConsumersController",name:"consumers"});a.otherwise({redirectTo:"/"})}])})();
